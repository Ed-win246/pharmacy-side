...
import { Plus, ListCheck, CircleCheckIcon, PackageOpen, Trash2,  } from 'lucide-vue-next';
...
async function submitStockBatch() {
    ...
    try {
        ...
        await api.post('/addstock', { items: stockItems.value });
        toast.success('Stock batch saved successfully!');
        stockItems.value = [];
        router.push('/viewstock');
    } catch (error) {
        console.error('Error saving stock batch:', error);
        toast.success('Stock saved locally in list! Ready for View Stock page integration.');
    } finally {
        saving.value = false;
    }
}
                ...
                <div class="bg-white rounded-lg shadow-sm p-5 border border-gray-200">
                    <form @submit.prevent="addItemToList" class="space-y-4">
                        ...
                        <button type="submit" class="w-full bg-green-600 hover:bg-green-700 rounded-sm items-center justify-center flex gap-2 px-4 py-2 text-white font-medium cursor-pointer transition">
                           <CircleCheckIcon class="w-4 h-4"/> Submit
                        </button>
                    </form>
                </div>
                <div>
                    <!-- Empty Placeholder Div -->
                    <div v-if="stockItems.length === 0" class="border-2 border-2 border-gray-300 rounded-lg p-10 text-center bg-gray-100 flex flex-col items-center justify-center min-h-[380px] space-y-3">
                        <div class="p-3 bg-gray-100 rounded-full text-gray-400">
                            <PackageOpen class="w-10 h-10 stroke-1" />
                        </div>
                    ...
                    <div v-else class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
                            <div class="text-xs text-gray-500">
                                Total Items: <span class="font-medium text-gray-800">{{ stockItems.length }}</span>
                            </div>
                            <button @click="submitStockBatch" :disabled="saving" class="bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-sm flex items-center gap-2 cursor-pointer transition">
